...
# roc_curve plots
roc_fig_dir_path = '/Users/jean-philippbruehwiler/Desktop/master-thesis-code/model-plots/xgbc-plots/xgbc-roc-plots/'
# shap plots
shap_fig_dir_path = '/Users/jean-philippbruehwiler/Desktop/master-thesis-code/model-plots/xgbc-plots/xgbc-shap-plots/'
...
param_grid = {
    ...
    'colsample_bytree': [0.17, 0.34, 0.5, 0.75, 1],  # subsample ratio of columns when constructing tree
    'gamma': [0, 0.1, 0.2],  # min loss reduction required to make a further partition on leaf node
    'reg_alpha': [0, 0.1, 0.2],
    'reg_lambda': [0, 0.1, 0.2]
}

# function to run svm model
def run_xgbc_model(data, lag, test_size, scoring, params):
    ...
    """This function takes the training data
       ...
       is extracted via grid search"""

    # make a copy of the original DataFrame to avoid modifying it
    data_copy = data.copy()

    # initiliaze the logistic regression model
    gradient_boosting_classifier = xgb.XGBClassifier(objective='binary:logistic', random_state=42, n_jobs=12)
    ...
    grid_search_cv = GridSearchCV(estimator=gradient_boosting_classifier, param_grid=params, cv=5, scoring=scoring)
    ...
    #roc_curve_plot(y_test, y_pred_raw, lag, roc_fig_dir_path, 'xgbc')
    
    # perform shap analyis on the model
    #shap_analysis(best_model, X_test, shap_fig_dir_path, 'xgbc', lag)
...
gradient_boosting_results = [(f"{lag}_month_lag_results", run_xgbc_model(data_raw, lag, 0.2, 'f1', param_grid)) for lag in lags]

# write data to excel to transfer to local file
# we will do further data processing in another script
path = '~/desktop/master-thesis-code/summary-xgbc-models.xlsx'
...
for result in gradient_boosting_results:
...
metric_data.to_excel(writer, sheet_name='xgbc-summary-stats', index=False)
...
complete_summary_stats_lg.to_excel(writer, sheet_name='xgbc-pos-pred-data', index=False)
# close writer
writer.close()

# export the model for import into another file
# define the export directory
export_dir = "~/Desktop/master-thesis-code/python-files/model-exports/xgbc-models"
# create directory if doesn't exist
os.makedirs(export_dir, exist_ok=True)
# loop over the models and extract each of them
for lag, model_data in zip(lags, gradient_boosting_results):
    # write the model to binary and export it using pickle
    with open(os.path.join(export_dir, f"xgbc-model-{lag}-month-lag.pkl"), "wb") as f:
