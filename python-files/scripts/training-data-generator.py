...
fred = Fred(api_key='[ENTER API KEY]')
...
# adjust file path
raw_sp500_data = pd.read_csv('[ENTER PATH TO SP500 CSV DATA]')
...
def calc_simple_moving_avg(data, window):
    ...
    a window size that defines the degree of
...
plt.show()
# looks good, highest level of correlation
# is only about 0.58 between consumer spending
# and consumer sentiment, but not anything to be
# too worried about
# just to be safe, we should do some further investigation
...
# seems fine, no VIF values above 5
# multicollinearity does not appear to be an issue among ind vars
